...
/** Paint a grayscale-mixed version of the bitmap onto a freshly-sized canvas. */
function paint(canvas: HTMLCanvasElement, bmp: ImageBitmap, intensity: number, bg: string | null) {
...
export function GrayscaleTool() {
  ...
  const repaint = useCallback(() => {
    if (previewRef.current && firstBmp.current) paint(previewRef.current, firstBmp.current, intensity, null);
  }, [intensity]);
  ...
  const applyAll = async () => {
    ...
    try {
      ...
      for (const it of items) {
        ...
        paint(canvas, bmp, intensity, mime === "image/jpeg" ? resolveBg(bg) : null);
        bmp.close();
        const blob = await canvasToBlob(canvas, mime, quality);
        out.push({ ...it, result: { blob, size: blob.size, name: `${baseName(it.file.name)}_grayscale.${mimeExt(mime)}` } });
      }
      setItems(out);
      setDone(true);
    ...
    <section className="grid grid-cols-1 lg:grid-cols-[1fr_340px] gap-6 items-start">
      ...
      <div className="lg:sticky lg:top-24 flex flex-col gap-4">
        <div className="bg-surface-container-lowest border border-surface-variant rounded-xl ambient-shadow p-5 flex flex-col gap-4">
          <h2 className="text-headline-md font-bold text-primary">Grayscale</h2>
        ...
        <button type="button" onClick={applyAll} disabled={isWorking} className="w-full inline-flex items-center justify-center gap-2 bg-secondary hover:bg-secondary-container text-on-secondary font-semibold py-3.5 rounded-lg transition-colors disabled:opacity-50">
          {isWorking ? (<><Icon name="progress_activity" className="animate-spin text-[20px]" /> Converting…</>) : (<><Icon name="filter_b_and_w" fill className="text-[20px]" /> Grayscale {items.length > 1 ? `${items.length} images` : "& download"}</>)}
        </button>

        {done && items.length > 1 && (
          <button type="button" onClick={() => zipAndDownload(items.filter((i) => i.result).map((i) => ({ name: i.result!.name, blob: i.result!.blob })), "omyimage_grayscale.zip")} className="w-full inline-flex items-center justify-center gap-2 border border-secondary text-secondary font-semibold py-2.5 rounded-lg hover:bg-secondary/10 transition-colors">
