...
import { formatEssay, pairCopy } from "@/lib/converters/i18n";
...
import { categoryNavLabel } from "@/lib/i18n/tool-labels";

/**
 * Format-conversion hub, in any language.
 *
 ...
 *
 * The cards read their names and the format essays from the same sources the
 * converter pages use (conversion.md §6.4), so a pair that is not translated
 * shows its English name here and links to its English page — the gate in
 * `status.ts` decides, not this file.
 */
...
export function ConverterHub({
  ...
  locale?: Locale;
  /** The locale's `pages/image-converter.ts` dictionary. Omit for English. */
  dict?: Dict;
  /** The <h1> and the last breadcrumb. */
  heading: string;
  /** The JSON-LD CollectionPage name — the long SEO title, not the <h1>. */
  schemaName: string;
  description: string;
}) {
  const t = getT(locale, dict);
  const home = localeHome(locale);
  const canonical = absoluteUrl(staticPath("/image-converter", locale));
  ...
  const collection = {
    ...
    hasPart: CONVERTER_PAIRS.map((p) => {
      ...
      return {
        ...
        name: pairCopy(p, locale, { seoTitle: "", seoDescription: "" }).name,
        url: absoluteUrl(toolHref(p.slug, locale)),
    ...
    <>
      <div data-tool-shell className="max-w-content mx-auto px-margin-mobile md:px-gutter pt-stack-md flex flex-col gap-stack-lg">
        ...
        <header className="flex flex-col gap-stack-sm mt-2">
          <h1 className="text-display-lg-mobile md:text-display-lg text-primary">{heading}</h1>
          <p className="text-body-lg text-on-surface-variant max-w-3xl">
            {t("{n} format converters, each one built for a specific pair rather than a generic drop-down. Pick your source format below. Everything is free, batch-capable, and runs in your browser unless the format makes that impossible.", { n: total })}
          </p>
        </header>
                  ...
                  >
                    ...
                    <span className="min-w-0">
                      <span className="block text-body-md font-semibold text-primary">
                        {pairCopy(p, locale, { seoTitle: "", seoDescription: "" }).name}
                      </span>
                      <span className="block text-label-sm font-label-sm text-on-surface-variant">
                        {t("to {format}", { format: to.prose })}
                      </span>
                    </span>
                  </Link>
                );
          ...
          <div className="flex flex-col gap-3 max-w-3xl">
            {FAQS.map((item) => (
              ...
              >
                ...
                <p className="px-6 pb-4 text-body-md text-on-surface-variant">{t(item.a)}</p>
              </details>
            ))}
          </div>
        </section>
